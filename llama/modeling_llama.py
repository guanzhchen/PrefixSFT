...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # self.non_causal = layer_idx in config.prefix_layer_list
...
class LlamaFlexAttention(LlamaAttention):
    ...
    def generate_block_prefix_causal_mask_mod(self, prompt_mask_4d, past_kv_len):
    ...
    def forward(
        ...
        prompt_mask = prompt_mask.expand(bsz, self.num_heads, q_len, q_len)
        # prompt_mask_4d = build_attention_mask_vectorized_simplified(prompt_mask).expand(bsz)
        mask_mod = self.generate_block_prefix_causal_mask_mod(prompt_mask, kv_len-q_len)
        block_mask = create_block_mask(mask_mod, None, None, q_len, kv_len, device=query_states.device, _compile=False)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.is_prefix_layer = layer_idx in config.prefix_layer_list
        attn_implementation = "flex_attention" if self.is_prefix_layer else config._attn_implementation
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.is_prefix_layer:
            kwargs.pop("prompt_mask", None)
        # Self Attention
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        prompt_mask = build_attention_mask_vectorized_simplified(prompt_mask)
